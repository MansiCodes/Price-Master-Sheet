...
import { canViewPnl, getAccessiblePlantIds, isPlantManager } from "@/lib/rbac";
...
export default async function ConsolidatedPnlPage({
  ...
  if (isPlantManager(session.user.globalRole)) {
    const plantIds = await getAccessiblePlantIds(session.user.id);
    redirect(plantIds[0] ? `/plants/${plantIds[0]}/pnl` : "/");
  }
