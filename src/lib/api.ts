...
export function requireCanEnterStock(
  role: GlobalRole,
): NextResponse | null {
  const enterDenied = requireCanEnter(role);
  if (enterDenied) return enterDenied;
  if (!canEnterStockData(role)) {
    return NextResponse.json(
      { error: "Accountants can only enter Purchase and Sales" },
      { status: 403 },
    );
  }
  return null;
...
export function requireCanEnterExpense(
  role: GlobalRole,
): NextResponse | null {
  const enterDenied = requireCanEnter(role);
  if (enterDenied) return enterDenied;
  if (!canEnterExpenseData(role)) {
    return NextResponse.json(
      { error: "Accountants can only enter Purchase and Sales" },
      { status: 403 },
    );
