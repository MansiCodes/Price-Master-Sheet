...
export type NavFlags = {
  showPnl: boolean;
  /** When true, Reports link is labeled Sales & Purchase (accountant limited view). */
  pnlSalesPurchaseOnly?: boolean;
...
export function getNavSections(flags: NavFlags): NavSection[] {
  ...
  if (showPnl && primaryPlantId) {
    reportItems.push({
      key: "pnl",
      label: pnlSalesPurchaseOnly ? "Sales & Purchase" : "P&L",
